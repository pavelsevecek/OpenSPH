...
TEST_CASE("StressForce Soundwave", "[stressforce]") {
    ...
    settings.set(GlobalSettingsIds::DOMAIN_SIZE, Vector(1.1_f, 1.1_f, 20.1_f));
    settings.set(GlobalSettingsIds::MODEL_FORCE_DIV_S, false);
    settings.set(GlobalSettingsIds::TIMESTEPPING_ADAPTIVE, true);
    settings.set(GlobalSettingsIds::TIMESTEPPING_INITIAL_TIMESTEP, 0._f);
    ...
    p.output = std::make_unique<TextOutput>("out_%d.txt", "wave",
        Array<QuantityIds>{ QuantityIds::POSITIONS, QuantityIds::DENSITY, QuantityIds::PRESSURE, QuantityIds::ENERGY });
