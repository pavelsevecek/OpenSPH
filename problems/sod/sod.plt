...
c1 = 0.44215
c2 = 0.27394
plot sprintf("%s.txt", filename) u 1:8 t "density" w lp ls 1, mid(x) not, post(x) not , h(x) not
plot sprintf("%s.txt", filename) u 1:4 t "velocity" w lp ls 2

c2 = 0.3190
plot sprintf("%s.txt", filename) u 1:7 t "pressure" w lp ls 3, post(x)
set yrange [1.5:3.]
plot sprintf("%s.txt", filename) u 1:9 t "energy" w lp ls 4
