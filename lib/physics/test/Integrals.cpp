...
TEST_CASE("Total momentum", "[integrals]") {
    ...
    storage->insert<QuantityKey::R, QuantityKey::M>();
    storage->get<QuantityKey::R>() = Array<Vector>{ Vector(1._f, 0._f, 0._f), Vector(0._f, 2._f, 0._f) };
    storage->dt<QuantityKey::R>()  = Array<Vector>{ Vector(0._f, 2._f, 0._f), Vector(0._f, 3._f, 0._f) };

    storage->get<QuantityKey::M>() = Array<Float>{ 5._f, 7._f };
