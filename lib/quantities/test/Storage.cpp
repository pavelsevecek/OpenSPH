...
TEST_CASE("Storage resize", "[storage]") {
    ...
    storage.emplace<Size, OrderEnum::ZERO_ORDER>(QuantityIds::MATERIAL_IDX, Array<Size>{ 0 });
    storage.resize(5);
    storage.emplace<Float, OrderEnum::FIRST_ORDER>(QuantityIds::DENSITY, 3._f);
    REQUIRE(storage.getQuantityCnt() == 2);
    REQUIRE(storage.getParticleCnt() == 5);

    storage.emplace<Vector, OrderEnum::SECOND_ORDER>(QuantityIds::MASSES, Vector(5._f));
...
TEST_CASE("Clone storages", "[storage]") {
    Storage storage;
    storage.emplace<Float, OrderEnum::ZERO_ORDER>(QuantityIds::MATERIAL_IDX, Array<Float>{ 0 });
    storage.resize(5);
    storage.emplace<Float, OrderEnum::SECOND_ORDER>(QuantityIds::POSITIONS, 4._f);
...
TEST_CASE("Storage merge", "[storage]") {
    Storage storage1;
    storage1.emplace<Float, OrderEnum::FIRST_ORDER>(QuantityIds::DENSITY, Array<Float>{ 0._f, 1._f });

    Storage storage2;
    storage2.emplace<Float, OrderEnum::FIRST_ORDER>(QuantityIds::DENSITY, Array<Float>{ 2._f, 3._f });
...
TEST_CASE("Storage init", "[storage]") {
    Storage storage;
    storage.emplace<Float, OrderEnum::ZERO_ORDER>(QuantityIds::MATERIAL_IDX, Array<Float>{ 0 }); // dummy unit
    storage.resize(3);
    storage.emplace<Float, OrderEnum::SECOND_ORDER>(QuantityIds::POSITIONS, 3._f);
    storage.emplace<Float, OrderEnum::FIRST_ORDER>(QuantityIds::MASSES, 1._f);
    storage.emplace<Float, OrderEnum::ZERO_ORDER>(QuantityIds::DENSITY, 2._f);
...
TEST_CASE("Storage material", "[storage]") {
    ...
    storage.emplace<Float, OrderEnum::FIRST_ORDER>(QuantityIds::DENSITY, 1._f);
    storage.emplace<Float, OrderEnum::FIRST_ORDER>(QuantityIds::ENERGY, 1._f);
...
TEST_CASE("Storage removeAll", "[storage]") {
    ...
    REQUIRE(storage.getParticleCnt() == 0);
    REQUIRE(storage.getQuantityCnt() == 0);
    storage.emplace<Float, OrderEnum::SECOND_ORDER>(QuantityIds::POSITIONS, { 3._f, 2._f, 5._f });
