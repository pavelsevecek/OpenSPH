...
TEST_CASE("EosAccessor", "[material]") {
    ...
    storage.insert<Float, OrderEnum::FIRST>(QuantityIds::DENSITY, Array<Float>{ 5._f });
    storage.insert<Float, OrderEnum::FIRST>(QuantityIds::ENERGY, Array<Float>{ 3._f });

    settings.set(BodySettingsIds::EOS, EosEnum::MURNAGHAN);
    Storage storage2(settings);
    storage2.insert<Float, OrderEnum::FIRST>(QuantityIds::DENSITY, Array<Float>{ 9._f });
    storage2.insert<Float, OrderEnum::FIRST>(QuantityIds::ENERGY, Array<Float>{ 6._f });
...
TEST_CASE("MaterialAccessor", "[material]") {
    ...
    storage.insert<Float, OrderEnum::FIRST>(QuantityIds::ENERGY, Array<Float>{ 1._f, 1._f });
    MaterialAccessor(storage).setParams(BodySettingsIds::YOUNG_MODULUS, 5._f);

    Storage storage2(BodySettings::getDefaults());
    storage2.insert<Float, OrderEnum::FIRST>(QuantityIds::ENERGY, Array<Float>{ 2._f, 2._f, 2._f });
