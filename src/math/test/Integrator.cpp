...
TEST_CASE("Integrators", "[integrators]") {
    ...
    REQUIRE(Math::almostEqual(result, Math::sphereVolume(1._f), targetError));
    ...
    REQUIRE(Math::almostEqual(result, 3 * 0.5_f, 2._f * targetError));

    // Gauss integral
    /*SphericalDomain<float, 2> circle(Vector<float, 2>(0.f), 1.f);
    Integrator<float, 2> int3(&circle);
    result =
        int3.integrate([](const Vector<float, 2>& v) { return Math::exp(-getSqrLength(v)); }, targetError);
    REQUIRE(Math::almostEqual(result,
                              (Math::E<float> - 1.f) / Math::E<float> * Math::PI<float>,
                              2.f * targetError));*/
