...
float FilmicMapping::eval(const float x) const {
    const float normX = x * m_invW;
...
void FilmicMapping::create(const DirectParams& srcParams) {
    ...
    float shoulderM = 0.0f;
    {
        ...
        */
        ...
        params.m_y0 = max(1e-5f, powf(params.m_y0, params.m_gamma));
        params.m_y1 = max(1e-5f, powf(params.m_y1, params.m_gamma));
